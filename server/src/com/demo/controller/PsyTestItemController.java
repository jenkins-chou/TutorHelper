...
@CrossOrigin
public class PsyTestItemController  extends Controller {
	
	public static final String DB_TABLE = "psy_test_item";
	...
	@CrossOrigin
	public void getAll(){
		List<PsyTestItemModel> models = PsyTestItemModel.dao.find("select * from "+DB_TABLE+" where del != 'delete'");
	...
	public void get(){
		...
		List<PsyTestItemModel> models = PsyTestItemModel.dao.find("select * from "+DB_TABLE+" where id = "+id+" and  del != 'delete'");
		JSONObject js = new JSONObject();
		if(models!=null&&models.size()>=1){
			js.put(Const.KEY_RES_CODE, Const.KEY_RES_CODE_200);
	...
	public void add(){
		JSONObject js = new JSONObject();
		try{
			PsyTestItemModel model = getModel(PsyTestItemModel.class, "", true);
	...
	@CrossOrigin
	public void search(){
		String map = "item_option";
		String key = getPara("key");
		List<PsyTestItemModel> models = PsyTestItemModel.dao.find("select * from "+DB_TABLE+" where "+map+" like '%"+key+"%' and del != 'delete'");
		JSONObject js = new JSONObject();
		if(models!=null && models.size()>=1){
			js.put(Const.KEY_RES_CODE, Const.KEY_RES_CODE_200);
			js.put(Const.KEY_RES_DATA, models);
