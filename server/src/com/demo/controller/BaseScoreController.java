...
@CrossOrigin
public class BaseScoreController  extends Controller {
	
	public static final String DB_TABLE = "base_score";
	
	public static final Map<String,String> tableFilter = new HashMap();
	
	static{
		tableFilter.put("id","hidden");
		tableFilter.put("course_id","custom");
		tableFilter.put("student_id","custom");
		tableFilter.put("teacher_id","custom");
	...
	@CrossOrigin
	public void getAll(){
		List<BaseScoreModel> models = BaseScoreModel.dao.find("select * from "+DB_TABLE+" where del != 'delete'");
	...
	public void get(){
		...
		List<BaseScoreModel> models = BaseScoreModel.dao.find("select * from "+DB_TABLE+" where id = "+id+" and  del != 'delete'");
		JSONObject js = new JSONObject();
		if(models!=null&&models.size()>=1){
			js.put(Const.KEY_RES_CODE, Const.KEY_RES_CODE_200);
	...
	public void add(){
		JSONObject js = new JSONObject();
		try{
			BaseScoreModel model = getModel(BaseScoreModel.class, "", true);
	...
	public void search(){
		...
		List<BaseScoreModel> models = BaseScoreModel.dao.find("select * from "+DB_TABLE+" where "+map+" like '%"+key+"%' and del != 'delete'");
		JSONObject js = new JSONObject();
		if(models!=null && models.size()>=1){
			js.put(Const.KEY_RES_CODE, Const.KEY_RES_CODE_200);
			js.put(Const.KEY_RES_DATA, models);
