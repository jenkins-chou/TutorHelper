...
@CrossOrigin
public class PsyTestAnswerController  extends Controller {
	
	public static final String DB_TABLE = "psy_test_answer";
	...
	static{
		...
		tableFilter.put("item_id","custom");
	...
	@CrossOrigin
	public void getAll(){
		List<PsyTestAnswerModel> models = PsyTestAnswerModel.dao.find("select * from "+DB_TABLE+" where del != 'delete'");
	...
	public void get(){
		...
		List<PsyTestAnswerModel> models = PsyTestAnswerModel.dao.find("select * from "+DB_TABLE+" where id = "+id+" and  del != 'delete'");
		JSONObject js = new JSONObject();
		if(models!=null&&models.size()>=1){
			js.put(Const.KEY_RES_CODE, Const.KEY_RES_CODE_200);
	...
	public void add(){
		JSONObject js = new JSONObject();
		try{
			PsyTestAnswerModel model = getModel(PsyTestAnswerModel.class, "", true);
	...
	public void search(){
		...
		List<PsyTestAnswerModel> models = PsyTestAnswerModel.dao.find("select * from "+DB_TABLE+" where "+map+" like '%"+key+"%' and del != 'delete'");
		JSONObject js = new JSONObject();
		if(models!=null && models.size()>=1){
			js.put(Const.KEY_RES_CODE, Const.KEY_RES_CODE_200);
			js.put(Const.KEY_RES_DATA, models);
