...
@CrossOrigin
public class PsyTestAnswerConsultController  extends Controller {
	
	public static final String DB_TABLE = "psy_test_answer_consult";
	
	public static final Map<String,String> tableFilter = new HashMap();
	
	static{
		tableFilter.put("id","hidden");
		tableFilter.put("subject_id","custom");
	...
	@CrossOrigin
	public void getAll(){
		List<PsyTestAnswerConsultModel> models = PsyTestAnswerConsultModel.dao.find("select * from "+DB_TABLE+" where del != 'delete'");
	...
	public void get(){
		...
		List<PsyTestAnswerConsultModel> models = PsyTestAnswerConsultModel.dao.find("select * from "+DB_TABLE+" where id = "+id+" and  del != 'delete'");
		JSONObject js = new JSONObject();
		if(models!=null&&models.size()>=1){
			js.put(Const.KEY_RES_CODE, Const.KEY_RES_CODE_200);
	...
	public void add(){
		JSONObject js = new JSONObject();
		try{
			PsyTestAnswerConsultModel model = getModel(PsyTestAnswerConsultModel.class, "", true);
	...
	public void search(){
		...
		List<PsyTestAnswerConsultModel> models = PsyTestAnswerConsultModel.dao.find("select * from "+DB_TABLE+" where "+map+" like '%"+key+"%' and del != 'delete'");
		JSONObject js = new JSONObject();
		if(models!=null && models.size()>=1){
			js.put(Const.KEY_RES_CODE, Const.KEY_RES_CODE_200);
			js.put(Const.KEY_RES_DATA, models);
