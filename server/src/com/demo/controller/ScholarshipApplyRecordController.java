...
@CrossOrigin
public class ScholarshipApplyRecordController  extends Controller {
	
	public static final String DB_TABLE = "scholarship_apply_record";
	
	public static final Map<String,String> tableFilter = new HashMap();
	
	static{
		tableFilter.put("id","hidden");
		tableFilter.put("user_id","custom");
	...
	@CrossOrigin
	public void getAll(){
		List<ScholarshipApplyRecordModel> models = ScholarshipApplyRecordModel.dao.find("select * from "+DB_TABLE+" where del != 'delete'");
	...
	public void get(){
		...
		List<ScholarshipApplyRecordModel> models = ScholarshipApplyRecordModel.dao.find("select * from "+DB_TABLE+" where id = "+id+" and  del != 'delete'");
		JSONObject js = new JSONObject();
		if(models!=null&&models.size()>=1){
			js.put(Const.KEY_RES_CODE, Const.KEY_RES_CODE_200);
	...
	public void add(){
		JSONObject js = new JSONObject();
		try{
			ScholarshipApplyRecordModel model = getModel(ScholarshipApplyRecordModel.class, "", true);
	...
	public void search(){
		...
		List<ScholarshipApplyRecordModel> models = ScholarshipApplyRecordModel.dao.find("select * from "+DB_TABLE+" where "+map+" like '%"+key+"%' and del != 'delete'");
		JSONObject js = new JSONObject();
		if(models!=null && models.size()>=1){
			js.put(Const.KEY_RES_CODE, Const.KEY_RES_CODE_200);
			js.put(Const.KEY_RES_DATA, models);
