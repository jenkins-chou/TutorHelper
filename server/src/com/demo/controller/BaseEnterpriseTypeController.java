...
public class BaseEnterpriseTypeController  extends Controller {
	...
	@CrossOrigin
	public void getAll(){
		List<BaseEnterpriseTypeModel> models = BaseEnterpriseTypeModel.dao.find("select * from "+DB_TABLE+" where del != 'delete'");
	...
	public void get(){
		...
		List<BaseEnterpriseTypeModel> models = BaseEnterpriseTypeModel.dao.find("select * from "+DB_TABLE+" where id = "+id+" and  del != 'delete'");
		JSONObject js = new JSONObject();
		if(models!=null&&models.size()>=1){
			js.put(Const.KEY_RES_CODE, Const.KEY_RES_CODE_200);
	...
	public void add(){
		JSONObject js = new JSONObject();
		try{
			BaseEnterpriseTypeModel model = getModel(BaseEnterpriseTypeModel.class, "", true);
