...
@CrossOrigin
public class BaseCollegeController  extends Controller {
	
	public static final String DB_TABLE = "base_college";
	...
	public void get(){
		...
		List<BaseCollegeModel> models = BaseCollegeModel.dao.find("select * from "+DB_TABLE+" where id = "+id+" and  del != 'delete'");
		JSONObject js = new JSONObject();
		if(models!=null&&models.size()>=1){
			js.put(Const.KEY_RES_CODE, Const.KEY_RES_CODE_200);
	...
	public void add(){
		JSONObject js = new JSONObject();
		try{
			BaseCollegeModel model = getModel(BaseCollegeModel.class, "", true);
