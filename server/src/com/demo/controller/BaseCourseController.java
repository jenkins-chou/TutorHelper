...
@CrossOrigin
public class BaseCourseController  extends Controller {
	
	public static final String DB_TABLE = "base_course";
	...
	@CrossOrigin
	public void getAll(){
		List<BaseCourseModel> models = BaseCourseModel.dao.find("select * from "+DB_TABLE+" where del != 'delete'");
	...
	public void get(){
		...
		List<BaseCourseModel> models = BaseCourseModel.dao.find("select * from "+DB_TABLE+" where id = "+id+" and  del != 'delete'");
		JSONObject js = new JSONObject();
		if(models!=null&&models.size()>=1){
			js.put(Const.KEY_RES_CODE, Const.KEY_RES_CODE_200);
	...
	public void add(){
		JSONObject js = new JSONObject();
		try{
			BaseCourseModel model = getModel(BaseCourseModel.class, "", true);
	...
	@CrossOrigin
	public void search(){
		String map = "course_name";
		String key = getPara("key");
		List<BaseCourseModel> models = BaseCourseModel.dao.find("select * from "+DB_TABLE+" where "+map+" like '%"+key+"%' and del != 'delete'");
		JSONObject js = new JSONObject();
		if(models!=null && models.size()>=1){
			js.put(Const.KEY_RES_CODE, Const.KEY_RES_CODE_200);
			js.put(Const.KEY_RES_DATA, models);
